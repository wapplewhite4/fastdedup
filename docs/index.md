...
I built [fastdedup](https://github.com/wapplewhite4/data-dedup), a Rust CLI for exactly this use case: fast, memory-efficient deduplication on a single machine. This post covers the benchmarks I ran against standard Python baselines on the FineWeb sample-10BT dataset.

---

## The Dataset

All benchmarks ran on [FineWeb sample-10BT](https://huggingface.co/datasets/HuggingFaceFW/fineweb), a well-known public subset of the full FineWeb dataset used as a standard reference in the ML community.
...
`fastdedup` uses character 3-grams + 128 MinHash signatures with 16 bands of 8 rows, matching standard FineWeb pipeline parameters. The comparison baseline is [datatrove](https://github.com/huggingface/datatrove) — the reference implementation used to produce FineWeb itself, making it the most credible possible comparison.
...
The tool is under active development. Feedback, issues, and contributions welcome on [GitHub](https://github.com/wapplewhite4/data-dedup).
